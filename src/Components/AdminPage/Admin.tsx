  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        const messagesData: Message[] = querySnapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => {
        ...
        <tbody>
          {messages.map((msg) => (
            <tr key={msg.id}>
              ...
              <td>{msg.phone}</td>
              <td>{msg.message}</td>
              <td>{msg.createdAt ? new Date(msg.createdAt.seconds * 1000).toLocaleString() : "N/A"}</td>
            </tr>
          ))}
        </tbody>
      </table>
